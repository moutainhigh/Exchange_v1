 ...
 * @version 1.0
 * @date 2014年4月23日
 * @Copyright: Copyright (c) 2014 深圳光汇云油电商有限公司
 * 
 */
public abstract class BaseTask implements Runnable {
	...
	public BaseTask(Context context, String processMsg, boolean cancelable,
		...
		if (needShow) {
			...
			this.cancelable = cancelable;
			if (!TextUtils.isEmpty(this.processMsg)) {
				ProcessDialogUtil.showDialog(context, processMsg,
						this.cancelable);
				ProcessDialogUtil.setOnDismissListener(new OnDismissListener() {
					@Override
					public void onDismiss(DialogInterface dialog) {
	...
	public void setDialog(Context context, String processMsg, boolean cancelable) {
		this.processMsg = processMsg;
		this.cancelable = cancelable;
		if (!TextUtils.isEmpty(this.processMsg)) {
			ProcessDialogUtil.showDialog(context, processMsg, this.cancelable);
...
//	}
	 ...
	 * @return 数据库操作返回数据对象
	 */
	public DataBaseRespon makeRespon(boolean isSuccess, String info,
			Object object) {
