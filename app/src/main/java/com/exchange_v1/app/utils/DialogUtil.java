...
import com.brightoilonline.c2b_phone.activity.mine.BankCardListActivity;
import com.brightoilonline.c2b_phone.activity2.car.AddOilActivity2;
...
import com.brightoilonline.c2b_phone.bean.UpdateBean;
import com.brightoilonline.c2b_phone.biz.MineBiz;
import com.brightoilonline.c2b_phone.config.BroadcastFilters;
...
import com.brightoilonline.c2b_phone.event.LoginEvent;
import com.brightoilonline.c2b_phone.event.MainMenuEvent;
import com.brightoilonline.c2b_phone.event.UpdateHomeEvent;
import com.brightoilonline.c2b_phone.network.RequestHandle;
import com.brightoilonline.c2b_phone.scan.MipcaActivityCapture;
import com.brightoilonline.c2b_phone.service.UpdateService;
import com.brightoilonline.c2b_phone.utils.ui.ViewUtils;
import com.brightoilonline.c2b_phone.utils.ui.textview.TVUtils;
import com.brightoilonline.c2b_phone.view.TinyOpenDialog;
import com.brightoilonline.c2b_phone.view.TwoButtonDialog;
import com.brightoilonline.c2b_phone.widget.CustomDialog;
import com.brightoilonline.c2b_phone.widget.DialogV273;
import com.brightoilonline.c2b_phone.widget.FuYouInputDialog;
import com.brightoilonline.c2b_phone.widget.HorizontalCustomDialog;
import com.brightoilonline.c2b_phone.widget.MyOilCardDialog;
import com.brightoilonline.c2b_phone.widget.NoGoodsDialog;
import com.brightoilonline.c2b_phone.widget.UpdateDialog;
...
public class DialogUtil {
     ...
     * @version 1.0
     */
    public static void showUpdateDg(final Context context, final UpdateBean bean) {
        if (!bean.getUrl().trim().replaceAll(" ", "").startsWith("http")) {
            return;
        }
        DialogV273 dialog = new DialogV273(context);
        dialog.setCancelable(false);
        if (bean == null
                || bean.getUrl().equals(
                UpdateManager.getVersionCode(context) - 0.1)) {
            dialog.setTitle(context
                    .getString(R.string.activity_setting_update_title));
            dialog.setMessage(context
                    .getString(R.string.activity_setting_update_isnew));
            dialog.hideCancelBtn("确定");
        } else {
            dialog.setBtnText("暂不升级", "马上升级");
            dialog.setCallback(new DialogV273.Callback() {
                @Override
                public void onCancel(DialogV273 dialog, View view) {
                    TApplication.updateBean = null;
                    TApplication.isShowUpdate = true;
                }

                @Override
                public void onConfirm(DialogV273 dialog, View view) {
                    ...
                    if (dialog.isIsNeedDismiss()) {
                        ...
                        final TextView tv = dialog.getComfirmBtn();
            ...
            String title = context
                    .getString(R.string.activity_setting_update_common_version, bean.getVersion_name());
            String messge = bean.getNote();
            //			int visioncode = UpdateManager
            //					.getVersionCode(context);// 本地版本号
            //			String Sversion=bean.getForceUpdteVer();
            //			String serviceVersion=Sversion.replace(".","");
            //			int version=Integer.valueOf(serviceVersion);
            if (bean.getIs_sure().equals("1")
                //					||version>visioncode
                    ) { // 强制更新
                //				title = context.getString(R.string.activity_setting_update_force);
                // 不可取消
                dialog.hideCancelBtnNoNeedDismiss();
            }
            if (StringUtil.isEmpty(messge)) {
                messge = context
                        .getString(R.string.activity_setting_update_hasnew);
            }

            dialog.setTitle(title);
            dialog.setMessage(messge);
            dialog.setMsgGravity(Gravity.LEFT);
        }
     ...
     * @param activity
     */
    public static void showFuYouPayDg(final Activity activity, final FuYouPayBean bean) {
        FuYouInputDialog dialog = new FuYouInputDialog(activity);
        dialog.setListener(new FuYouInputDialog.OnMyClickListener() {
            @Override
            public void onClick(String bankcard_no, String name, String idcard) {
                bean.setBANKCARD(bankcard_no.replace(" ", ""));
                bean.setNAME(name);
                bean.setIDNO(idcard);

                bean.setIDTYPE("0");
                PayUtil.fuyou_pay(activity, bean);
            }
        });
        dialog.show();
     ...
     */
    public static void showTwoBtnDialog(Context context, String title, String msg, String cancelStr, String confirmStr, TwoButtonDialog.OnClickListener onClickListener){
        TwoButtonDialog twoButtonDialog = new TwoButtonDialog(context);
        twoButtonDialog.showDialog(title, msg, cancelStr, confirmStr, onClickListener);
    }
    ...
    public static void showMyOilCardDg(Context context) {
        MyOilCardDialog dialog = new MyOilCardDialog(context);
        ...
        HorizontalCustomDialog dialog = new HorizontalCustomDialog.Builder(
                ...
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        //登出后，清除手势数据
                        //						GestureUtil.putString(userNo,"");
                        //						GestureUtil.putBoolean(userNo+"_able",false);
                        GestureUtil.clearCode(userNo);
                        GestureUtil.putIsAutoString(TApplication.context.getString(R.string.spkey_value_gestrue_autologin), false);
    ...
    private static void logout(Context context) {
        MineBiz.logout(context, new RequestHandle() {
            @Override
     ...
     * @updateInfo (此处输入修改内容, 若无修改可不写.)
     */
    public static void showNotBandleBankCardDg(final Context context) {

        HorizontalCustomDialog dialog = new HorizontalCustomDialog.Builder(
                context).setTitle("未绑定银行卡").setMessage("是否立即绑定银行卡？")
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {

                    @Override
                    public void onClick(DialogInterface dialog, int which) {

                        IntentUtil.gotoActivity(context, BankCardListActivity.class);
                        dialog.dismiss();
                    }
                })
                .setNegativeButton("取消", new DialogInterface.OnClickListener() {
     ...
     * 商品列表无数据时，显示无数据界面
     *
     * @param context message 消息提示内容文本， confir_str,确定按钮上显示的文本 cancle_str,取消按钮上显示的文本
     *                state 状态量 0表示领取操作，1，表示转让操作，2，表示回购操作
     */
    public static NoGoodsDialog showNoGoodsDialog(final Context context,
                                                  SpannableStringBuilder message, String confir_str, String cancle_str) {
        NoGoodsDialog dialog = new NoGoodsDialog(context);
     ...
     * @version 1.0
     * @createTime 2013-11-25,下午9:50:43
     * @updateTime 2013-11-25,下午9:50:43
     * @createAuthor liujingguo
     * @updateAuthor liujingguo
     * @updateInfo (此处输入修改内容, 若无修改可不写.)
     */
    public static void showOffLineDg(final Context context,
                                     final boolean isClose) {
        UserInfoUtil.logout();
        CustomDialog dialog = new CustomDialog(context);
        dialog.setTitle(context.getString(R.string.prompt));
        dialog.setMessage(context.getString(R.string.warning_out_login));

        dialog.setSingleBtn(new CustomDialog.OnClickListener() {
            ...
            public void onClick(CustomDialog dialog, int id, Object object) {
                ...
                bundle.putBoolean(
                        context.getString(R.string.intent_key_islogin), false);
                if (isClose) {
                    IntentUtil.gotoActivityAndFinish(context,
                            LoginActivity2.class, bundle);
                } else {
                    IntentUtil.gotoActivity(context, LoginActivity2.class,
                            bundle);
                }
                dialog.dismiss();
            }
...
//    }
     ...
     * @updateInfo (此处输入修改内容, 若无修改可不写.)
     */
    public static HorizontalCustomDialog showNoOilIdDg(final Context mycontext, String message, final boolean quitActivity) {

        HorizontalCustomDialog dialog = new HorizontalCustomDialog.Builder(
                mycontext).setTitle("错误提示").setMessage(message)
                .setPositiveButton("取消", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        ((AddOilActivity2) mycontext).finish();

                    }
                })
                .setNegativeButton("继续扫码", new DialogInterface.OnClickListener() {

                    @Override
                    public void onClick(DialogInterface dialog, int which) {


                        IntentUtil.gotoActivity(mycontext, MipcaActivityCapture.class);
                        ((AddOilActivity2) mycontext).finish();
                    }
                }).setPositiveBgId(R.drawable.shape_dia_right_gray).setPositiveTextColor(R.color.title_bg_gray).setNegativeBgId(R.drawable.shape_dia_left_red).setNegativeTextColor(R.color.white).create();
        dialog.show();
     ...
     * @param onClickListener
     */
    public static void showTinyOpenDialog(Context context, String title, String msg, String confirmStr, TinyOpenDialog.OnClickListener onClickListener){
        TinyOpenDialog tinyOpenDialog = new TinyOpenDialog(context);
        tinyOpenDialog.showDialog(title, msg, confirmStr, onClickListener);
    }
