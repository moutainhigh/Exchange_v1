...
public class AsyncImageloader {
	...
	/** 是否第一次取原图 */
	// private boolean isLoadRoundPhoto = true;
	 ...
	 * 
	 * @param maxThread 最大允许线程数
	 * @param resId 默认图片资源Id，加载图片失败的时候将返回该图片
	 */
	...
	public void setPortraitDefaul(ImageView image) {
		if (image != null && defaultBitmap != null) {
	 ...
	 * @param context 上下文
	 * @param position 位置
	 * @param photoId 图片在相册里边的id
	 * @param callback 图片下载完成后回调
	 * @param isMediaPhoto 是否从系统相册加载图片
	 * @return 返回软引用图片
	 */
	 ...
	 * 
	 * @version 1.0
	 * @createTime 2015年2月12日,上午11:16:45
	 * @updateTime 2015年2月12日,上午11:16:45
	 ...
	 * @return
	 */
	public Bitmap loadImageNoDfBitmap(final Context context, final ImageView image, final int position, final String imagePath,
	 ...
	 * @return 返回软引用图片
	 */
	public Bitmap loadImageBitmapLocal(final Context context, final ImageView image, final int position, final String imagePath,
                                       final String localPath, final ImageCallback callback, final boolean isPortrait, final String str) {
		// 如果缓存过就从缓存中取出数据
		final Handler handler = new Handler() {
			@Override
	...
	private Bitmap loadImageFromPath(String imagePath, int size, int radius, final String savePath) {
		...
		if (isVideo) {// 如果是视屏，获取视频缩略图
			...
			if (new File(loaclPath).exists()) {
				...
				// 如果是头像，先判断头像是否有，如果没有头像缩略图，直接使用原图
				if (isPortrait && !Net.checkURL(imagePath) && (imagePath.contains("_150x150") || imagePath.contains("_300x300"))) {
					if (imagePath.contains("_150x150")) {
						imagePath = imagePath.replace("_150x150.jpg", ".jpg");
					} else if (imagePath.contains("_300x300")) {
						imagePath = imagePath.replace("_300x300.jpg", ".jpg");
					}
				}
